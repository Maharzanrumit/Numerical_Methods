...
int main() {
    ...
    // Display x, y, x^2, x^3, x^4, x*y, x^2*y
    cout << "x\t" << "y\t" << "x^2\t" << "x^3\t" << "x^4\t" << "x*y\t" << "x^2*y\n";
    for (i = 0; i < n; i++) {
        cout << arrx[i] << "\t" << arry[i] << "\t" << pow(arrx[i], 2) << "\t" << pow(arrx[i], 3)
             << "\t" << pow(arrx[i], 4) << "\t" << arrx[i] * arry[i] << "\t" << pow(arrx[i], 2) * arry[i] << endl;
    ...
    // Output the sums
    cout << "Ex: " << sumx << "\tEy: " << sumy << "\tEx^2: " << sum_sqrx << "\tEx^3: " << sum_cubex
         << "\tEx^4: " << sum_x4 << "\tExy: " << sumxy << "\tEx^2*y: " << sum_sqrx_y << endl;
